...
def run_rt_gsg_simple_sse_lp():
    ...
    print(np.mean([round(x,4) for x in df.support]))
    print(stats.sem([round(x,4) for x in df.support]))
    print(np.mean([round(x,4) for x in df.runtime]))
    print(stats.sem([round(x,4) for x in df.runtime]))
    print(np.mean([round(x,4) for x in df.defender_utility]))
    print(stats.sem([round(x,4) for x in df.defender_utility]))

def run_rm_gsg_general_sse_lp():
    schedule_form_kwargs = {
        "schedule_form": True,
        "simple": False,
        "attacker_penalty_factor": 5,
...
def run_rts_gsg_general_sse_lp():
    schedule_form_kwargs = {
        "schedule_form": True,
        "simple": False,
        "attacker_penalty_factor": 5,
...
def run_rts_isg_general_sse_lp():
    schedule_form_kwargs = {
        "schedule_form": True,
        "simple": False,
        "attacker_penalty_factor": 3,
